...
float3 GetBrushColor(float3 location)
{
    ...
    if(_brushType == 2) //원
    {
        ...
        //float dist = distance(float2(location.x, location.z), float2(_brushLocation.x, _brushLocation.z));
...
PixelInput VS(VertexInput input)
{
    ...
    output.brushColor = float4(GetBrushColor(input.position.xyz), 0);
...
float4 PS(PixelInput input) : SV_TARGET
{
    ...
    return diffuseMap * intensity + input.brushColor;
}